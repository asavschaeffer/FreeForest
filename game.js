    ...
    function showEndScreen() {
        ...
        projectiles = [];
        towers = [];
        log(`Game state changed to end at frame ${frameCount}`, 'system');
    }
    ...
    function pauseGame() {
        if (gameState === 'playing') {
            ...
            if (animationFrameId) {
                ...
                log(`Animation frame cancelled: ${animationFrameId}`, 'system');
                animationFrameId = null;
            }
            log(`Game state changed to paused at frame ${frameCount}`, 'system');
        }
    }
    ...
    function resumeGame() {
        if (gameState === 'paused') {
            gameState = 'playing';
            pauseScreen.style.display = 'none';
            log(`Game state changed to resumed at frame ${frameCount}`, 'system');
            lastTime = performance.now();
            lastSpawnTime = lastTime;
            animationFrameId = requestAnimationFrame(gameLoop);
            log(`Animation frame requested: ${animationFrameId}`, 'system');
        }
    }
    ...
    canvas.addEventListener('click', (e) => {
        ...
        log(`Mouse clicked at (${Math.round(clickX)}, ${Math.round(clickY)})`, 'system');
        ...
        for (const enemy of enemies) {
            if (clickX >= enemy.x && clickX <= enemy.x + enemy.width &&
                ...
                targetEnemy = enemy;
                log(`Clicked on enemy at (${Math.round(enemy.x)}, ${Math.round(enemy.y)})`, 'system');
                break;
            }
        }
    ...
    function Projectile(startX, startY, targetX, targetY) {
        ...
        if (distance === 0) {
            log(`Warning: distance is zero in Projectile for start (${startX}, ${startY}) to target (${targetX}, ${targetY})`, 'system');
        ...
        if (isNaN(this.velocity.x) || isNaN(this.velocity.y)) {
            log(`Warning: NaN velocity in Projectile for start (${startX}, ${startY}) to target (${targetX}, ${targetY})`, 'system');
            this.velocity = { x: 0, y: 0 };
        }
    ...
    function calculateVelocityTowardPlayer(x, y, speed) {
        ...
        if (distance === 0) {
            log(`Warning: distance is zero in calculateVelocityTowardPlayer for position (${x}, ${y})`, 'system');
            return { x: 0, y: 0 };
        }
        ...
        if (isNaN(velocity.x) || isNaN(velocity.y)) {
            log(`Warning: NaN velocity calculated for position (${x}, ${y})`, 'system');
            return { x: 0, y: 0 };
        }
    ...
    function gameLoop(currentTime) {
        ...
        if (frameCount % 60 === 0) {
            log(`Player position: (${Math.round(player.x)}, ${Math.round(player.y)})`, 'player');
            enemies.forEach((enemy, index) => {
                log(`Enemy ${index} position: (${Math.round(enemy.x)}, ${Math.round(enemy.y)})`, 'enemy');
            });
            towers.forEach((tower, index) => {
                log(`Tower ${index} position: (${Math.round(tower.x)}, ${Math.round(tower.y)})`, 'tower');
            });
            projectiles.forEach((projectile, index) => {
                log(`Projectile ${index} position: (${Math.round(projectile.x)}, ${Math.round(projectile.y)})`, 'projectile');
            });
            log(`Delta time: ${deltaTime}ms`, 'system');
        }
        ...
        enemies.forEach(enemy => {
            ...
            if (!player.invulnerable &&
                ...
                log(`Collision detected between player at (${Math.round(player.x)}, ${Math.round(player.y)}) and enemy at (${Math.round(enemy.x)}, ${Math.round(enemy.y)})`, 'system');
            ...
            towers.forEach(tower => {
                if (enemy.x < tower.x + tower.width &&
                    ...
                    log(`Collision detected between enemy at (${Math.round(enemy.x)}, ${Math.round(enemy.y)}) and tower at (${Math.round(tower.x)}, ${Math.round(tower.y)})`, 'system');
                    tower.health--;
                    enemy.health--;
                    if (tower.health <= 0) {
